...
├── 📁 notebooks/                     # 📚 Optional learning materials
│   └── 📄 01_data_prep.ipynb        # ETL pipeline documentation (not required)
...
#### 🔬 **Analysis Files**
- **`notebooks/01_data_prep.ipynb`** - **Optional educational resource**:
  - 📚 **Purpose**: Documents the complete data processing pipeline
  - 🎯 **Usage**: Understanding, learning, and customizing ETL logic
  - ⚠️ **Note**: Not required for app functionality (processing is automatic)
  - 🛠️ **When to use**: Data exploration, pipeline modification, or learning
...
#### 5. **Launch the Dashboard** (No manual data processing required)
```bash
streamlit run app/app.py
```

**🎯 Smart Data Processing**: The app automatically processes raw data on first launch! No need to run the notebook manually.
...
### 🔧 Optional: Manual Data Processing

If you prefer to process data manually or want to understand the ETL pipeline:
...
# This will generate the same df_final.csv file
```
...
|------|-------------|---------|-----------------|
| `data/df_final.csv` | App on first launch | Processed unified dataset for dashboard | ❌ No (auto-generated) |
| `.streamlit/` | Streamlit | App configuration cache | ❌ No (excluded) |
| `__pycache__/` | Python | Compiled bytecode | ❌ No (excluded) |

**🎯 Key Improvement**: The app now automatically processes data, so no manual notebook execution is required for basic usage!

### 🔧 Troubleshooting

#### Common Issues:

**1. App takes time to load on first launch:**
```bash
# Solution: This is normal! The app processes data automatically on first run
# You'll see a "Processing data..." spinner - just wait a moment
```
